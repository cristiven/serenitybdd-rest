...
> USE test_automation;
> CREATE TABLE example ( id smallint unsigned not null auto_increment, name varchar(20) not null, constraint pk_example primary key (id) ); INSERT INTO example ( id, name ) VALUES ( null, 'Pepito Vargas' );
> SELECT * FROM example;
> DESCRIBE example;
...
> In the root folder add a folder called: resources, inside add  a folder: META-INF and create the file: persistence.xml
 ...
 <class>Example</class>  (provide the entity package path)
